...
#define CAMSV_DEBUG_DISABLE_SOF_INT 0
	...
	unsigned int sub_ratio)
{
	...
	// #ifdef OPLUS_FEATURE_CAMERA_COMMON
	sv_dev->ts_ns = 0;
	atomic_set(&sv_dev->abnormal_sof_cnt, 0);
	atomic_set(&sv_dev->is_irq_panic, 0);
	// #endif /*OPLUS_FEATURE_CAMERA_COMMON*/
...
static irqreturn_t mtk_irq_camsv_sof(int irq, void *data)
{
	...
	if (irq_info.sof_tags & sv_dev->last_tag) {
		if (sv_dev->ts_ns) {
			if (irq_info.ts_ns - sv_dev->ts_ns < 1000000) {
				atomic_inc(&sv_dev->abnormal_sof_cnt);
				if (atomic_read(&sv_dev->abnormal_sof_cnt) == 3) {
#if CAMSV_DEBUG_DISABLE_SOF_INT
					dev_info(sv_dev->dev, "disable camsv sof interrupt\n");
					/* disable sof interrupt */
					writel_relaxed(0, sv_dev->base_inner + REG_CAMSVCENTRAL_SOF_STATUS_EN);
#endif
				} else if (atomic_read(&sv_dev->abnormal_sof_cnt) == 5) {
					if (atomic_read(&sv_dev->is_irq_panic) == 0) {
						atomic_set(&sv_dev->is_irq_panic, 1);
						irq_info.is_irq_panic = 1;
					}
				}
			}
		}
		sv_dev->ts_ns = irq_info.ts_ns;
	}
	// #endif /*OPLUS_FEATURE_CAMERA_COMMON*/
...
static irqreturn_t mtk_thread_irq_camsv(int irq, void *data)
{
	...
	while (kfifo_len(&sv_dev->msg_fifo) >= sizeof(irq_info)) {
		...
		// #ifdef OPLUS_FEATURE_CAMERA_COMMON
		if (irq_info.is_irq_panic) {
			/* dummy reg. w/r */
			writel_relaxed(0xDEADBEEF, sv_dev->base_inner + REG_CAMSVCENTRAL_SPARE);
			dev_info(sv_dev->dev, "ts=%llu irq_type %d, req:0x%x/0x%x, tg_cnt:%d spare:0x%x\n",
				irq_info.ts_ns / 1000,
				irq_info.irq_type,
				irq_info.frame_idx_inner,
				irq_info.frame_idx,
				irq_info.tg_cnt,
				readl_relaxed(sv_dev->base_inner + REG_CAMSVCENTRAL_SPARE));
