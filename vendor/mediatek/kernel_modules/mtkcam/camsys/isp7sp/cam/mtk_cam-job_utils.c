...
{
	u32 line = get_sensor_h(job) * CAM_TUNING_BEGIN_F_RATIO / 100 +
		CAM_TUNING_DELAY_NS / get_line_time(job);

	return min(line, get_sensor_h(job));
}

u32 get_tuning_end_line(struct mtk_cam_job *job)
{
	u32 duration = CAM_TUNING_DEADLINE_NS / get_line_time(job);

	return min(get_tuning_begin_line(job) + duration, get_sensor_h(job));
}
