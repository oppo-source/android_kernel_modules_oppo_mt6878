...
static int reset_yuv_msgfifo(struct mtk_yuv_device *dev);
...
{
	dev_info(dev->dev,
		 "%s [outer] TG SENMODE/VFCON/PATHCFG/VSEOL_SUB: %x/%x/%x/%x FRMSIZE/R GRABPXL/LIN: %x/%x %x/%x MISC:%x\n",
		 msg,
		 ...
		 readl_relaxed(dev->base + REG_CAMCTL_MISC));

	dev_info(dev->dev,
		 "%s [inner] TG SENMODE/VFCON/PATHCFG/VSEOL_SUB: %x/%x/%x/%x GRABPXL/LIN: %x/%x MISC:%x\n",
		 msg,
		 ...
		 readl_relaxed(dev->base_inner + REG_CAMCTL_MISC));
}
...
#define CAMCTL_MISC_DEFAULT 0x8014
		...
		struct engine_callback *cb)
{
	...
	/* misc settings */
	writel_relaxed(CAMCTL_MISC_DEFAULT, dev->base + REG_CAMCTL_MISC);
...
void lock_done_ctrl_enable(struct mtk_raw_device *dev, int on)
{
	...
	SET_FIELD(&val, CAMCTL_LOCK_DONE_SEL, on);
	writel(val , dev->base + REG_CAMCTL_MISC);

	dev->lock_done_ctrl = on;

	dev_info(dev->dev, "[%s] raw%d - on:%d misc:0x%x\n",
		__func__, dev->id, on, readl(dev->base + REG_CAMCTL_MISC));
}
...
static int push_yuv_msgfifo(struct mtk_yuv_device *dev,
			struct mtk_camsys_irq_info *info)
{
	int len;
...
static irqreturn_t mtk_irq_raw(int irq, void *data)
{
	...
	unsigned int drop_status, dma_ofl_status, cq_done_status, dcif_status, ctrl_misc;
	unsigned int dma_ufl_status;
	unsigned int tg_cnt, lock_done;
	bool wake_thread = 0;
	...
	ctrl_misc = readl_relaxed(raw_dev->base_inner + REG_CAMCTL_MISC);
	...
	if (CAM_DEBUG_ENABLED(RAW_INT))
		dev_info(dev,
			 "INT:0x%x(err:0x%x) 2~7 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x misc:0x%x (in:0x%x) tg_cnt:%d/%lld\n",
			 irq_status, err_status,
			 dmao_done_status, dmai_done_status, drop_status,
			 dma_ofl_status, cq_done_status, dcif_status, ctrl_misc,
	...
	/* ois compensation */
	if (raw_dev->lock_done_ctrl &&
		dmao_done_status & FBIT(CAMCTL_FHO_R1_DONE_ST)) {
			lock_done = readl_relaxed(raw_dev->base + REG_CAMCTL_MISC);
			SET_FIELD(&lock_done, CAMCTL_LOCK_DONE_SEL, 1);
			writel(lock_done , raw_dev->base + REG_CAMCTL_MISC);
	}
...
static irqreturn_t mtk_thread_irq_yuv(int irq, void *data)
{
	struct mtk_yuv_device *yuv_dev = (struct mtk_yuv_device *)data;
	struct mtk_raw_device *raw_dev = get_raw_dev(data);
	struct engine_fsm *fsm = &raw_dev->fsm;
	struct mtk_camsys_irq_info irq_info;
	int cookie_done;

	if (unlikely(atomic_cmpxchg(&yuv_dev->is_fifo_overflow, 1, 0)))
		dev_info(yuv_dev->dev, "msg fifo overflow\n");

	while (kfifo_len(&yuv_dev->msg_fifo) >= sizeof(irq_info)) {
		int len = kfifo_out(&yuv_dev->msg_fifo, &irq_info, sizeof(irq_info));

		WARN_ON(len != sizeof(irq_info));

		if (engine_fsm_hw_done(fsm, &cookie_done) > 0)
			irq_info.cookie_done = cookie_done;
...
static irqreturn_t mtk_irq_yuv(int irq, void *data)
{
	...
	struct mtk_raw_device *raw = get_raw_dev(data);
	struct mtk_camsys_irq_info irq_info;
	unsigned int frame_idx, frame_idx_inner;
	bool wake_thread = 0;
	...
	frame_idx	= readl_relaxed(raw->base + REG_FRAME_SEQ_NUM);
	frame_idx_inner	= readl_relaxed(raw->base_inner + REG_FRAME_SEQ_NUM);
	...
	if (CAM_DEBUG_ENABLED(RAW_INT))
		if (irq_status || wdma_done_status || err_status) {
			dev_info(yuv->dev, "YUV-INT:0x%x(err:0x%x) INT2/4/5 0x%x/0x%x/0x%x, in/out:0x%x/0x%x\n",
			irq_status, err_status,
			wdma_done_status, drop_status, dma_ofl_status, frame_idx, frame_idx_inner);
	}
	...
	/* Frame done */
	if (wdma_done_status & FBIT(CAMCTL2_RZH1N2TO_R2_DONE_ST)) {
		if (raw->lock_done_ctrl && !yuv->is_slave) {
			irq_info.irq_type |= 1 << CAMSYS_IRQ_FRAME_DONE;
			if (push_yuv_msgfifo(yuv, &irq_info) == 0)
				wake_thread = 1;
		}
	}
...
static int mtk_yuv_probe(struct platform_device *pdev)
{
	...
	drvdata->msg_buffer = devm_kzalloc(dev, drvdata->fifo_size, GFP_KERNEL);
	if (!drvdata->msg_buffer) {
		ret = -ENOMEM;
		goto UNREGISTER_PM_NOTIFIER;
	}
