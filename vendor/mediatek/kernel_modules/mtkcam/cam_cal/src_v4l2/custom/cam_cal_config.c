...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static void parsing_ois_cal_data_in_preload(unsigned char *eeprom_preload)
{
	if (eeprom_preload == NULL) {
		return;
	}
	...
	if (!do_set_ois_once) {
		...
		cam_cal_config->parsing_ois_cal_data_from_preload(eeprom_preload, &preload_ois);
	...
	if (cam_cal_config->enable_preload && bufSize) {
		...
		if (mp_eeprom_preload[preloadIndex] == NULL) {
			...
			if (read_data_region(pdata, mp_eeprom_preload[preloadIndex],
				...
				parsing_ois_cal_data_in_preload(mp_eeprom_preload[preloadIndex]);
			}
#endif /*OPLUS_FEATURE_CAMERA_COMMON*/
		}
		if (!(mp_eeprom_preload[preloadIndex] == NULL ||
...
{
	unsigned char ois_table_new[OTP_SIZE] = {0};
	int i = 0;
	int j = 0;

	*is_valid = (preload_ois.valid == 0x01) ? true : false;
	if (*is_valid) {
		for (i = 0; i < OTP_INDEX_NUM; i++) {
			ois_table_new[i * (OTP_TBL_NUM * 2)] = preload_ois.ois_table[i * (OTP_TBL_NUM * 2)];
			ois_table_new[i * (OTP_TBL_NUM * 2) + 1] = preload_ois.ois_table[i * (OTP_TBL_NUM * 2) + 1];
			for (j = 1; j < OTP_TBL_NUM; j++) {
				ois_table_new[i * (OTP_TBL_NUM * 2) + j * 2] =
					preload_ois.ois_table[i * (OTP_TBL_NUM * 2) + (OTP_TBL_NUM - j) * 2];
				ois_table_new[i * (OTP_TBL_NUM * 2) + j * 2 + 1] =
					preload_ois.ois_table[i * (OTP_TBL_NUM * 2) + (OTP_TBL_NUM - j) * 2 + 1];
			}
		}
	}
	if (d_data)
		memcpy(d_data, ois_table_new, sizeof(ois_table_new));
	return;
}
