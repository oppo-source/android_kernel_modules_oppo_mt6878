...
#include "ac230_p_3_a0004_dsi_cmd_panel_t1.h"
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	DISP_ERR("ac230_p_3_a0004_t1 %s +, mode id=%d\n", __func__, mode_id);
	//panel_power_on = true;
	switch (mode_id) {
	case FHD_SDC60:
...
//	.corner_pattern_lt_addr = (void *)top_rc_pattern,
		...
		.dsc_params = {
			...
			.dec_delay = 571,
			.scale_value = 32,
			.increment_interval = 525,
			.decrement_interval = 8,
			.line_bpg_offset = 12,
			.nfl_bpg_offset = 1294,
			.slice_bpg_offset = 1101,
			.initial_offset = 6144,
			.final_offset = 4304,
			.flatness_minqp = 3,
			.flatness_maxqp = 12,
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	//unsigned char bl_level_page[] = {0xFF,0x5A,0xA5,0x00};
	...
	DISP_ERR("[%s:%d]backlight bl_level oplus_debug NO LHBM\n", __func__, __LINE__);
	...
	//cb(dsi, handle, bl_level_page, ARRAY_SIZE(bl_level_page));
	cb(dsi, handle, bl_level, ARRAY_SIZE(bl_level));
	DISP_ERR("ac230_p_3_a0004_t1 backlight = %d bl_level[1]=%x, bl_level[2]=%x\n", level, bl_level[1], bl_level[2]);
...
static int oplus_esd_backlight_recovery(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	//unsigned char default_bl_page[] = {0xFF, 0x5A, 0xA5, 0x00};
	...
	//cb(dsi, handle, default_bl_page, ARRAY_SIZE(default_bl_page));
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	DISP_ERR("%s: ac230_p_3_a0004_t1 poweron Start\n", __func__);
	//iovcc enable 1.8V
	...
	DISP_ERR("%s: ac230_p_3_a0004_t1 poweron Successful\n", __func__);
	return 0;
}
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	DISP_ERR("%s: ac230_p_3_a0004_t1 poweroff Start\n", __func__);
	...
	if (ret < 0)
		lcm_unprepare(panel);
	DISP_ERR("%s: ac230_p_3_a0004_t1 poweroff Successful\n", __func__);
	return 0;
}
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	pr_err("[LCM] %s+ ac230_p_3_a0004_t1 Start\n", __func__);
	...
	register_device_proc("lcd", "AC230_A0004", "P_3");
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	pr_err("ac230_p_3_a0004_t1 lcm probe End.\n");
	return ret;
}
...
static const struct of_device_id lcm_of_match[] = {
	{
		.compatible = "ac230,p,3,a0004,cmd,t1",
	},
	{}
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "ac230_p_3_a0004_dsi_cmd_t1",
		.owner = THIS_MODULE,
...
MODULE_AUTHOR("oplus");
MODULE_DESCRIPTION("ac230,p,3,a0004,cmd,t1,OLED Driver");
