...
#include "panel_ac230_p_7_a0014_dsi_cmd.h"
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	DISP_ERR("panel_ac230_p_7_a0014 %s +, mode id=%d\n", __func__, mode_id);
	//panel_power_on = true;
	switch (mode_id) {
	case FHD_SDC60:
	...
	DISP_ERR("panel_ac230_p_7_a0014 %s -\n", __func__);
}
...
static int panel_lhbm_pressed_icon_grayscale_update(void *para_list, unsigned int bl_level)
{
	...
	unsigned char tx_buf_1[3] = {0xB8, 0x08, 0x00};
	int rc = 0;
	...
	if (!oplus_panel_pwm_onepulse_is_enabled()) {
		if (((last_backlight == 0x0000) || (last_backlight > 0x0520) || pwm_is_changing)
				&& ((bl_level > 0x0000) && (bl_level <= 0x0520))) {
			memcpy(tx_buf, tx_buf_0, 3);
			need_to_update_grayscale = true;
		} else if (((last_backlight <= 0x0520) || (last_backlight > 0x0852) || pwm_is_changing)
						&& ((bl_level > 0x0520) && (bl_level <= 0x0852))) {
			tx_buf[1] = (2048 * (bl_level - 1314) / (2130 - 1314) ) >> 8;
			tx_buf[2] = (2048 * (bl_level - 1314) / (2130 - 1314) ) & 0xFF;
			need_to_update_grayscale = true;
		} else if (((last_backlight <= 0x0851) || (last_backlight > 0x0DBB) || pwm_is_changing)
						&& ((bl_level > 0x0851) && (bl_level <= 0x0DBB))) {
			memcpy(tx_buf, tx_buf_1, 3);
			need_to_update_grayscale = true;
		}
	} else {
		if (((last_backlight == 0x0000) || (last_backlight > 0x0520) || pwm_is_changing)
			...
			tx_buf[2] = (2048 * (bl_level - 1314) / (2130 - 1314) ) & 0xFF;
			need_to_update_grayscale = true;
		} else if (((last_backlight <= 0x074C) || (last_backlight > 0x0FFE) || pwm_is_changing)
						&& ((bl_level > 0x074C) && (bl_level <= 0x0FFE))) {
			memcpy(tx_buf, tx_buf_1, 3);
			need_to_update_grayscale = true;
		}
	}
	...
	if (need_to_update_grayscale) {
		OFP_INFO("lhbm pressed icon grayscale:0x%02X, 0x%02X\n", tx_buf[1],  tx_buf[2]);
		rc = 1;
	}
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	if (oplus_ofp_is_supported()) {
		if (lhbm_pressed_icon_grayscale_cmd[2].count == 3) {
...
//	}
	...
	DISP_ERR("panel_ac230_p_7_a0014 backlight = %d bl_level[1]=%x, bl_level[2]=%x\n", level, bl_level[1], bl_level[2]);
...
static int vddr8_buck4_regulator_enable(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(mt6319_8_vbuck4)) {
		ret = regulator_set_voltage(mt6319_8_vbuck4, 1190000, 1193750);
		if (ret < 0)
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	DISP_ERR("%s: panel_ac230_p_7_a0014_dsi_cmd_panel poweron Successful\n", __func__);
	return 0;
}
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	if (ret < 0)
		lcm_unprepare(panel);
	DISP_ERR("%s: panel_ac230_p_7_a0014_dsi_cmd_panel poweroff Successful\n", __func__);
	return 0;
}
...
static int lcm_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	mode[1]->type = DRM_MODE_TYPE_DRIVER | DRM_MODE_TYPE_PREFERRED;
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	pr_err("[LCM] %s+ panel_ac230_p_7 a0014 Start\n", __func__);
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	pr_err("panel_ac230_P_7_A0014_dsi_cmd_panel lcm probe End.\n");
	return ret;
}
...
static const struct of_device_id lcm_of_match[] = {
	{
		.compatible = "ac230,p,7,a0014,cmd",
	},
	{}
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel_ac230_p_7_a0014_dsi_cmd",
		.owner = THIS_MODULE,
...
MODULE_AUTHOR("oplus");
MODULE_DESCRIPTION("panel,ac230,p,7,a0014,OLED Driver");
