...
struct cpumask silver_cpus;
struct cpumask gold_cpus;
...
static void setup_cluster_cpus(void)
{
	int min_cap = SCHED_CAPACITY_SCALE;

	for (int i = 0; i < ux_sched_cputopo.cls_nr; ++i) {
		cpumask_or(&all_cpus, &all_cpus, &ux_sched_cputopo.sched_cls[i].cpus);

		if (ux_sched_cputopo.sched_cls[i].capacity < min_cap)
			min_cap = ux_sched_cputopo.sched_cls[i].capacity;
	}

	for (int i = 0; i < ux_sched_cputopo.cls_nr; ++i) {
		if (ux_sched_cputopo.sched_cls[i].capacity == min_cap)
			cpumask_or(&silver_cpus, &silver_cpus, &ux_sched_cputopo.sched_cls[i].cpus);
	}

	cpumask_andnot(&gold_cpus, &all_cpus, &silver_cpus);
}
...
void android_rvh_set_cpus_allowed_by_task_handler(void *unused, const struct cpumask *cpu_valid_mask, const struct cpumask *new_mask,
	...
	/* TODO: not clear why wechat been tied to little cluster, so just prohibit it here */
	if (is_top(p) && (!strncmp(p->comm, "com.tencent.mm", 14)) && cpumask_any_and_distribute(new_mask, &gold_cpus) >= nr_cpu_ids) {
		if (cpumask_any_and_distribute(&tmp_mask, &gold_cpus) >= nr_cpu_ids) {
			cpumask_copy((struct cpumask *)new_mask, &all_cpus);
			if (unlikely(global_debug_enabled & DEBUG_FTRACE))
				pr_info("top task(%d %d) been tied to little cluster, reset to full cpus\n", p->pid, current->pid);
		} else {
			cpumask_copy((struct cpumask *)new_mask, &tmp_mask);
			if (unlikely(global_debug_enabled & DEBUG_FTRACE))
				pr_info("top task(%d %d) been tied to little cluster, reset to origin cpus\n", p->pid, current->pid);
		}
