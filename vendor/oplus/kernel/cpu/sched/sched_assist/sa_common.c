...
bool im_mali(const char *comm)
{
	return !strcmp(comm, "mali-event-hand") ||
		...
		!strcmp(comm, "mali-compiler") || !strcmp(comm, "ged-swd");
}
bool tast_special_task(struct task_struct *p)
{
	return !strcmp(p->group_leader->comm, "ndroid.systemui");
}
#endif
...
void sched_assist_target_comm(struct task_struct *task, const char *comm)
{
	...
	if (im_mali(comm)) {
		if (tast_special_task(task))
			oplus_set_ux_state_lock(task, SA_TYPE_HEAVY, -1, true);
		else
