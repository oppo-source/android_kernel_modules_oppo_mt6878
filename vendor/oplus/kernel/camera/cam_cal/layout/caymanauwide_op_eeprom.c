...
#define PFX "CAM_CAL_CAYMANAUWIDE"
...
static unsigned int do_single_lsc_caymanauwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_caymanauwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_caymanauwide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static unsigned int layout_check_caymanauwide(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x016d009a, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00000530, 0x0000074C, do_single_lsc_caymanauwide},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_caymanauwide},  //Start address, block size is useless
		...
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_caymanauwide}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT caymanauwide_op_eeprom = {
	.name = "caymanauwide_op_eeprom",
	.check_layout_function = layout_check_caymanauwide,
...
static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &caymanauwide_op_eeprom;
...
static unsigned int layout_check_caymanauwide(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID)
{
	...
	if (check_id == 0x016B012B || check_id == 0x016d009a) {	// hearder id on OTP guide
		...
		printk("caymanaheader_id wide matched 0x%08x\n", check_id);
		result = CAM_CAL_ERR_NO_ERR;
	} else{
		debug_log("header_id not matched 0x%08x\n", check_id);
		printk("caymanaheader_id wide not matched 0x%08x\n", check_id);
	}
	return result;
}
