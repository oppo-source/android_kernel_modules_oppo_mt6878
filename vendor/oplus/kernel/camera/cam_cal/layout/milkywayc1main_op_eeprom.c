...
#define PFX "CAM_CAL_MILKYWAYC1MAIN"
...
static unsigned int do_single_lsc_milkywayc1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_milkywayc1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_milkywayc1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_milkywayc1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01480126, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00001D60, 0x0000074C, do_single_lsc_milkywayc1main},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_milkywayc1main}, //Start address, block size is useless
		{0x00000001, 0x000024B0, 0x00000A3A, do_pdaf_milkywayc1main},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_milkywayc1main}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT milkywayc1main_op_eeprom = {
	.name = "milkywayc1main_op_eeprom",
...
#define MILKYWAYC1MAIN_HVBIN_PDAF_PROC1_SIZE  (976)
#define MILKYWAYC1MAIN_HVBIN_PDAF_PROC2_SIZE  (1624)
#define MILKYWAYC1MAIN_HVBIN_PDAF_PROC1_ADDR  (0x24B0)
#define MILKYWAYC1MAIN_HVBIN_PDAF_PROC2_ADDR  (0x2890)

unsigned int do_pdaf_milkywayc1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = MILKYWAYC1MAIN_HVBIN_PDAF_PROC1_SIZE + MILKYWAYC1MAIN_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MILKYWAYC1MAIN_HVBIN_PDAF_PROC1_ADDR + MILKYWAYC1MAIN_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MILKYWAYC1MAIN_HVBIN_PDAF_PROC2_ADDR + MILKYWAYC1MAIN_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MILKYWAYC1MAIN_HVBIN_PDAF_PROC2_ADDR, MILKYWAYC1MAIN_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[MILKYWAYC1MAIN_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
