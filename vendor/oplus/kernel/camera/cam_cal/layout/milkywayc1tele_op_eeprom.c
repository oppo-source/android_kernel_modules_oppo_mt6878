...
#define PFX "CAM_CAL_MILKYWAYC1TELE"
...
static unsigned int do_single_lsc_milkywayc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_milkywayc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_milkywayc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_milkywayc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_ois_milkywayc1tele(unsigned char *eeprom_preload,
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01ae010d, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000B00, 0x0000074C, do_single_lsc_milkywayc1tele},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_milkywayc1tele}, //Start address, block size is useless
		{0x00000001, 0x00001300, 0x00000A3A, do_pdaf_milkywayc1tele},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_milkywayc1tele}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT milkywayc1tele_op_eeprom = {
	.name = "milkywayc1tele_op_eeprom",
	...
	.preload_size = 0x8000,
	.has_stored_data = 1,
	.parsing_ois_cal_data_from_preload = do_ois_milkywayc1tele,
};


#define MILKYWAYC1TELE_HVBIN_PDAF_PROC1_SIZE  (496)
#define MILKYWAYC1TELE_HVBIN_PDAF_PROC2_SIZE  (1004)
#define MILKYWAYC1TELE_HVBIN_PDAF_PROC1_ADDR  (0x1300)
#define MILKYWAYC1TELE_HVBIN_PDAF_PROC2_ADDR  (0x1500)

unsigned int do_pdaf_milkywayc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = MILKYWAYC1TELE_HVBIN_PDAF_PROC1_SIZE + MILKYWAYC1TELE_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MILKYWAYC1TELE_HVBIN_PDAF_PROC1_ADDR + MILKYWAYC1TELE_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MILKYWAYC1TELE_HVBIN_PDAF_PROC2_ADDR + MILKYWAYC1TELE_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MILKYWAYC1TELE_HVBIN_PDAF_PROC2_ADDR, MILKYWAYC1TELE_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[MILKYWAYC1TELE_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
