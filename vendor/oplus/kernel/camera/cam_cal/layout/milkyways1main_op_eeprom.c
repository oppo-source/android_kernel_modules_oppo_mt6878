...
#define PFX "CAM_CAL_MILKYWAYS1MAIN"
...
static unsigned int do_single_lsc_milkyways1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_milkyways1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_milkyways1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_milkyways1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01480126, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00001D60, 0x0000074C, do_single_lsc_milkyways1main},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_milkyways1main}, //Start address, block size is useless
		{0x00000001, 0x000024B0, 0x00000A3A, do_pdaf_milkyways1main},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_milkyways1main}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT milkyways1main_op_eeprom = {
	.name = "milkyways1main_op_eeprom",
	.check_layout_function = layout_check,
	.read_function = Common_read_region,
	.layout = &cal_layout_table,
	.sensor_id = MILKYWAYS1MAIN_SENSOR_ID,
	.i2c_write_id = 0xA0,
	.max_size = 0x8000,
	.enable_preload = 1,
	.preload_size = 0x8000,
...
#define MILKYWAYS1MAIN_HVBIN_PDAF_PROC1_SIZE  (976)
#define MILKYWAYS1MAIN_HVBIN_PDAF_PROC2_SIZE  (1624)
#define MILKYWAYS1MAIN_HVBIN_PDAF_PROC1_ADDR  (0x24B0)
#define MILKYWAYS1MAIN_HVBIN_PDAF_PROC2_ADDR  (0x2890)

unsigned int do_pdaf_milkyways1main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = MILKYWAYS1MAIN_HVBIN_PDAF_PROC1_SIZE + MILKYWAYS1MAIN_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MILKYWAYS1MAIN_HVBIN_PDAF_PROC1_ADDR + MILKYWAYS1MAIN_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MILKYWAYS1MAIN_HVBIN_PDAF_PROC2_ADDR + MILKYWAYS1MAIN_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MILKYWAYS1MAIN_HVBIN_PDAF_PROC2_ADDR, MILKYWAYS1MAIN_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[MILKYWAYS1MAIN_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
