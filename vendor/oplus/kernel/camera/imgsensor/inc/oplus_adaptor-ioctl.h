...
static int g_set_af_code_data(struct adaptor_ctx *ctx, void *arg)
{
	struct oplus_set_af_code_data *info = arg;
	union feature_para para;
	u32 len;

	para.u64[0] = info->af_code_macro;
	para.u64[1] = info->af_code_infinity;
...
static int g_get_ois_compensation(struct adaptor_ctx *ctx, void *arg)
{
	struct oplus_ois_compensation_data *info = arg;
	union feature_para para;
	u32 len;
	u8  ois_compensation = 0;
	para.u64[0] = (u64)&(info->sensor_mode);
	para.u64[1] = (u64)&ois_compensation;

	subdrv_call(ctx, feature_control,
		SENSOR_FEATURE_GET_OIS_COMPENSATION,
		para.u8, &len);
	if (copy_to_user((void *)info->p_ois_compensation, &ois_compensation, sizeof(ois_compensation)))
		return -EFAULT;
	return 0;
}

static int g_get_ois_otp_info(struct adaptor_ctx *ctx, void *arg)
{
	struct oplus_ois_otp_info *info = arg;
	union feature_para para;
	u32 len;
	struct SENSOR_OIS_OTP_STRUCT otp_info;
	memset(&otp_info, 0, sizeof(otp_info));
	para.u64[0] = (u64)&otp_info;

	subdrv_call(ctx, feature_control,
		SENSOR_FEATURE_GET_OIS_OTP_INFO,
		para.u8, &len);
	pr_info("%s sensor_id:%d ois_otp_info exist:%u", __func__, info->sensor_id, otp_info.is_ois_otp_data_exist);
	if (copy_to_user((void *)info->p_ois_info, &otp_info, sizeof(otp_info))) {
		return -EFAULT;
	}
	return 0;
}
...
static const struct ioctl_entry oplus_ioctl_list[] = {
	...
	{VIDIOC_MTK_S_AF_CODE_DATA, g_set_af_code_data},
	{VIDIOC_MTK_G_OIS_COMPENSATION, g_get_ois_compensation},
	{VIDIOC_MTK_G_OIS_OTP_INFO, g_get_ois_otp_info},
};
