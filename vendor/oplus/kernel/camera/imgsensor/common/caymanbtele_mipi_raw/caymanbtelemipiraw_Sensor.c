 ...
 *============================================================================
 ****************************************************************************/
#include "caymanbtelemipiraw_Sensor.h"

#define SENSOR_NAME  SENSOR_DRVNAME_CAYMANBTELE_MIPI_RAW
#define PFX "caymanbtele_camera_sensor"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)
#define CAYMANBTELE_EEPROM_READ_ID     (0xA1)
#define CAYMANBTELE_EEPROM_WRITE_ID    (0xA0)
...
#define CAYMANBTELE_STEREO_MT_START_ADDR  (0x22C0)
#define CAYMANBTELE_STEREO_MT105_START_ADDR  (0x2640)
#define CAYMANBTELE_AESYNC_START_ADDR     (0x2290)
#define CAYMANBTELE_EEPROM_LOCK_REGISTER  (0xE000)
...
static int caymanbtele_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_set_test_pattern_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_get_eeprom_comdata(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static void caymanbtele_set_gain_convert(struct subdrv_ctx *ctx, u32 gain);
static int caymanbtele_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void caymanbtele_set_multi_gain(struct subdrv_ctx *ctx, u32 *gains, u16 exp_cnt);
static void caymanbtele_set_hdr_tri_gain(struct subdrv_ctx *ctx, u64 *gains, u16 exp_cnt);
static int caymanbtele_set_hdr_tri_gain2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_set_hdr_tri_gain3(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static void caymanbtele_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter);
static int caymanbtele_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void caymanbtele_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
static void caymanbtele_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt, u16 frame_length);
static int caymanbtele_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_extend_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_set_video_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
// static int caymanbtele_set_sensor_rmsc_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_i2c_burst_wr_regs_u16(struct subdrv_ctx *ctx, u16 * list, u32 len);
static int adapter_i2c_burst_wr_regs_u16(struct subdrv_ctx * ctx,
		u16 addr, u16 *list, u32 len);
static void caymanbtele_lens_pos_writeback(struct subdrv_ctx *ctx);
static int caymanbtele_set_register(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int caymanbtele_get_register(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static bool g_id_from_dts_flag = false;
static void get_imgsensor_id_from_dts(struct subdrv_ctx *ctx, u32 *sensor_id);
/* STRUCT */

// static struct oplus_get_eeprom_common_data caymanbtele_eeprom_common_data = {0};
static kal_uint16 g_af_code_macro    = 0;
static kal_uint16 g_af_code_infinity = 0;
// static kal_uint16 g_af_code_middle   = 0;

static struct eeprom_map_info caymanbtele_eeprom_info[] = {
	...
	{ EEPROM_META_STEREO_MT_MAIN_DATA, CAYMANBTELE_STEREO_MT_START_ADDR, 0, 0, CALI_DATA_SLAVE_TELE_LENGTH, false },
	{ EEPROM_META_STEREO_MT_MAIN_DATA_105CM, CAYMANBTELE_STEREO_MT105_START_ADDR, 0, 0, CALI_DATA_SLAVE_TELE_LENGTH, false },
};

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, caymanbtele_set_test_pattern},
	{SENSOR_FEATURE_SET_TEST_PATTERN_DATA, caymanbtele_set_test_pattern_data},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, caymanbtele_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, caymanbtele_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, caymanbtele_get_eeprom_comdata},
	{SENSOR_FEATURE_SET_SENSOR_OTP, caymanbtele_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, caymanbtele_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, caymanbtele_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, caymanbtele_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, caymanbtele_streaming_resume},
	{SENSOR_FEATURE_SET_GAIN, caymanbtele_set_gain},
	{SENSOR_FEATURE_SET_DUAL_GAIN, caymanbtele_set_hdr_tri_gain2},
	{SENSOR_FEATURE_SET_HDR_TRI_GAIN, caymanbtele_set_hdr_tri_gain3},
	{SENSOR_FEATURE_SET_ESHUTTER, caymanbtele_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, caymanbtele_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, caymanbtele_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, caymanbtele_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, caymanbtele_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, caymanbtele_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_SET_SEAMLESS_EXTEND_FRAME_LENGTH, caymanbtele_extend_frame_length},
	{SENSOR_FEATURE_SET_VIDEO_MODE, caymanbtele_set_video_mode},
	// {SENSOR_FEATURE_SET_SENSOR_RMSC_MODE, caymanbtele_set_sensor_rmsc_mode},
	{SENSOR_FEATURE_SET_AWB_GAIN, caymanbtele_set_awb_gain},
	{SENSOR_FEATURE_SET_REGISTER, caymanbtele_set_register},
	{SENSOR_FEATURE_GET_REGISTER, caymanbtele_get_register},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{/*4sum12.5Mp_30FPS_4096x3072*/
		...
		.mode_setting_table = caymanbtele_preview_capture_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_preview_capture_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = caymanbtele_bining_12_5M_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbtele_bining_12_5M_seamless_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 920000000,
	...
	{/*4sum4k_30FPS_4096x2304*/
		...
		.mode_setting_table = caymanbtele_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_normal_video_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbtele_bining_4k_30fps_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbtele_bining_4k_30fps_seamless_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 920000000,
	...
	{/*4sum4k_60FPS_4096x2304*/
		...
		.mode_setting_table = caymanbtele_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sum2bin_120FPS_2048x1152*/
		...
		.mode_setting_table = caymanbtele_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sum12.5Mp_24FPS_4096x3072*/
		...
		.mode_setting_table = caymanbtele_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sum2bin_24FPS_2048x1536*/
		...
		.mode_setting_table = caymanbtele_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*full50Mp_15FPS_8192x6144_remosaicON*/
		...
		.mode_setting_table = caymanbtele_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sum4k_30FPS_4096x2304_2expSHDR*/
		...
		.mode_setting_table = caymanbtele_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom4_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = caymanbtele_bining_4k_30fps_shdr_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbtele_bining_4k_30fps_shdr_seamless_setting),
	...
	{/*fullcrop12.5Mp_30FPS_4096x3072_remosaicON*/
		...
		.mode_setting_table = caymanbtele_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom5_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sum2bin_30FPS_2048x1152*/
		...
		.mode_setting_table = caymanbtele_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom6_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sum2bin_240FPS_2048x1152*/
		...
		.mode_setting_table = caymanbtele_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom7_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sumFHD_30FPS_2048x1152_2expSHDR*/
		...
		.mode_setting_table = caymanbtele_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_STAGGER,
	...
	{/*4sum2bin_30FPS_2048x1536*/
		...
		.mode_setting_table = caymanbtele_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sum2bin_15FPS_2048x1536*/
		...
		.mode_setting_table = caymanbtele_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom10_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sum2bin_15FPS_2048x1152*/
		...
		.mode_setting_table = caymanbtele_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom11_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* 4sum4k_30FPS_3264x1836 */
		...
		.mode_setting_table = caymanbtele_custom12_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom12_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* 4sum4k_15FPS_3264x1836 */
		...
		.mode_setting_table = caymanbtele_custom13_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom13_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*4sum12.5M_30FPS_4096x3072_2expSHDR*/
		...
		.mode_setting_table = caymanbtele_custom14_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom14_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_STAGGER,
	...
	{/*fullcrop12.5Mp_30FPS_4096x3072_remosaic0ff*/
		...
		.mode_setting_table = caymanbtele_custom15_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom15_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = caymanbtele_izoom_qbc_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(caymanbtele_izoom_qbc_seamless_setting),
	...
	{/*4sum2bin_30FPS_680x512*/
		...
		.mode_setting_table = caymanbtele_custom16_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom16_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* 4sum4k_30FPS_3264x2448 */
		...
		.mode_setting_table = caymanbtele_custom17_setting,
		.mode_setting_len = ARRAY_SIZE(caymanbtele_custom17_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = CAYMANBTELE_SENSOR_ID,
	...
	.ana_gain_step = 2,
	.ana_gain_table = caymanbtele_ana_gain_table,
	.ana_gain_table_size = sizeof(caymanbtele_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
...
//	.init_setting_table = caymanbtele_sensor_init_setting,
//	.init_setting_len =  ARRAY_SIZE(caymanbtele_sensor_init_setting),
	.mode = mode_struct,
...
struct subdrv_entry caymanbtele_mipi_raw_entry = {
	.name = SENSOR_NAME,
	.id = CAYMANBTELE_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int caymanbtele_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			caymanbtele_set_shutter_convert(ctx, *(u32 *)para);
...
static unsigned int read_caymanbtele_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != caymanbtele_eeprom_info[meta_id].meta)
		return -1;

	if (size != caymanbtele_eeprom_info[meta_id].size)
		return -1;

	addr = caymanbtele_eeprom_info[meta_id].start;
	readsize = caymanbtele_eeprom_info[meta_id].size;
...
// 	memset(&caymanbtele_eeprom_common_data, 0x00, sizeof(caymanbtele_eeprom_common_data));

// 	caymanbtele_eeprom_common_data.header[EEPROM_MODULE_ID] = 2;
// 	read_caymanbtele_eeprom_info(ctx, EEPROM_META_MODULE_ID,
// 				&(caymanbtele_eeprom_common_data.data[idx]), 2);
// 	idx += caymanbtele_eeprom_common_data.header[EEPROM_MODULE_ID];

// 	caymanbtele_eeprom_common_data.header[EEPROM_SENSOR_ID] = 2;
// 	read_caymanbtele_eeprom_info(ctx, EEPROM_META_SENSOR_ID,
// 				&(caymanbtele_eeprom_common_data.data[idx]), 2);
// 	idx += caymanbtele_eeprom_common_data.header[EEPROM_SENSOR_ID];

// 	caymanbtele_eeprom_common_data.header[EEPROM_LENS_ID] = 2;
// 	read_caymanbtele_eeprom_info(ctx, EEPROM_META_LENS_ID,
// 				&(caymanbtele_eeprom_common_data.data[idx]), 2);
// 	idx += caymanbtele_eeprom_common_data.header[EEPROM_LENS_ID];

// 	caymanbtele_eeprom_common_data.header[EEPROM_VCM_ID] = 2;
// 	read_caymanbtele_eeprom_info(ctx, EEPROM_META_VCM_ID,
// 				&(caymanbtele_eeprom_common_data.data[idx]), 2);
// 	idx += caymanbtele_eeprom_common_data.header[EEPROM_VCM_ID];

// 	caymanbtele_eeprom_common_data.header[EEPROM_MODULE_SN] = 23;
// 	read_caymanbtele_eeprom_info(ctx, EEPROM_META_MODULE_SN,
// 				&(caymanbtele_eeprom_common_data.data[idx]), 23);
// 	idx += caymanbtele_eeprom_common_data.header[EEPROM_MODULE_SN];

// 	caymanbtele_eeprom_common_data.header[EEPROM_AF_CODE_MACRO] = 2;
// 	caymanbtele_eeprom_common_data.header[EEPROM_AF_CODE_INFINITY] = 2;
// 	caymanbtele_eeprom_common_data.header[EEPROM_AF_CODE_MIDDLE] = 2;
// 	read_caymanbtele_eeprom_info(ctx, EEPROM_META_AF_CODE,
// 				&(caymanbtele_eeprom_common_data.data[idx]), 6);

// 	AF_CODE_MACRO_idx = idx;
// 	AF_CODE_INFINITY_idx = idx + 2;
// 	AF_CODE_MIDDLE_idx = idx + 4;

// 	g_af_code_macro = caymanbtele_eeprom_common_data.data[AF_CODE_MACRO_idx + 1] << 8 |
// 		caymanbtele_eeprom_common_data.data[AF_CODE_MACRO_idx];
// 	g_af_code_infinity = caymanbtele_eeprom_common_data.data[AF_CODE_INFINITY_idx + 1] << 8 |
// 		caymanbtele_eeprom_common_data.data[AF_CODE_INFINITY_idx];;
// 	g_af_code_middle = caymanbtele_eeprom_common_data.data[AF_CODE_MIDDLE_idx + 1] << 8 |
// 		caymanbtele_eeprom_common_data.data[AF_CODE_MIDDLE_idx];;

// 	for (idx = 0; idx < 64; idx = idx + 4)
// 		LOG_INF("In %s:common data: %02x %02x %02x %02x\n", __func__,
// 			caymanbtele_eeprom_common_data.data[idx], caymanbtele_eeprom_common_data.data[idx + 1],
// 			caymanbtele_eeprom_common_data.data[idx + 2],
// 			caymanbtele_eeprom_common_data.data[idx + 3]);
// }
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, CAYMANBTELE_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, CAYMANBTELE_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
{
    kal_int32 ret = ERROR_NONE;
    kal_uint16 reg = CAYMANBTELE_EEPROM_LOCK_REGISTER;
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, CAYMANBTELE_EEPROM_WRITE_ID >> 1,
            reg, 0xA1);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, CAYMANBTELE_EEPROM_WRITE_ID >> 1,
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == CAYMANBTELE_SENSOR_ID)
            && (data_length == CALI_DATA_SLAVE_TELE_LENGTH)
            && ((data_base == CAYMANBTELE_STEREO_MT_START_ADDR) || (data_base == CAYMANBTELE_STEREO_MT105_START_ADDR))) {
        ...
        } else if ((pStereodata->uSensorId == CAYMANBTELE_SENSOR_ID)
            && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == CAYMANBTELE_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                    read_cmos_eeprom_8(ctx, CAYMANBTELE_AESYNC_START_ADDR),
                    read_cmos_eeprom_8(ctx, CAYMANBTELE_AESYNC_START_ADDR+1),
                    read_cmos_eeprom_8(ctx, CAYMANBTELE_AESYNC_START_ADDR+2),
                    read_cmos_eeprom_8(ctx, CAYMANBTELE_AESYNC_START_ADDR+3),
                    read_cmos_eeprom_8(ctx, CAYMANBTELE_AESYNC_START_ADDR+4),
                    read_cmos_eeprom_8(ctx, CAYMANBTELE_AESYNC_START_ADDR+5),
                    read_cmos_eeprom_8(ctx, CAYMANBTELE_AESYNC_START_ADDR+6),
                    read_cmos_eeprom_8(ctx, CAYMANBTELE_AESYNC_START_ADDR+7));
...
static int caymanbtele_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case EEPROM_STEREODATA_MT_MAIN:
		read_caymanbtele_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MT_MAIN_105CM:
		read_caymanbtele_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA_105CM,
				(BYTE *)feature_return_para_32, *len);
		break;
	default:
		break;
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CAYMANBTELE_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
static int open(struct subdrv_ctx *ctx)
{
	...
	if ((module_flag & 0xFF00) == 0x0300){
		...
		subdrv_i2c_wr_regs_u16(ctx, caymanbtele_sensor_init_pre_setting1, ARRAY_SIZE(caymanbtele_sensor_init_pre_setting1));
		mdelay(5);
		subdrv_i2c_wr_regs_u16(ctx, caymanbtele_sensor_init_pre_setting2, ARRAY_SIZE(caymanbtele_sensor_init_pre_setting2));
		mdelay(5);
		caymanbtele_i2c_burst_wr_regs_u16(ctx, caymanbtele_sensor_simple_init_setting, ARRAY_SIZE(caymanbtele_sensor_simple_init_setting));
		...
		subdrv_i2c_wr_regs_u16(ctx, caymanbtele_sensor_init_pre_setting2, ARRAY_SIZE(caymanbtele_sensor_init_pre_setting2));
		mdelay(5);
		subdrv_i2c_wr_regs_u16(ctx, caymanbtele_sensor_init_setting, ARRAY_SIZE(caymanbtele_sensor_init_setting));
		DRV_LOGE(ctx, "setting end\n");
	};
...
//	ctx->extend_frame_length_en = FALSE;
	...
	if (ae_ctrl) {
		...
		case HDR_RAW_STAGGER:
			caymanbtele_set_multi_shutter_frame_length(ctx, (u64*)&ae_ctrl->exposure, exp_cnt, 0);
			caymanbtele_set_multi_gain(ctx, (u32*)&ae_ctrl->gain, exp_cnt);
			break;
		default:
			caymanbtele_set_shutter_convert(ctx, ae_ctrl->exposure.le_exposure);
			caymanbtele_set_gain_convert(ctx, ae_ctrl->gain.le_gain);
			break;
		}
	}
...
void caymanbtele_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
...
static void caymanbtele_set_hdr_tri_gain(struct subdrv_ctx *ctx, u64 *gains, u16 exp_cnt)
{
	...
	caymanbtele_set_multi_gain(ctx,	values, exp_cnt);
}

static int caymanbtele_set_hdr_tri_gain2(struct subdrv_ctx *ctx, u8 *para, u32 *len) {

	u64 *feature_data = (u64 *) para;
	caymanbtele_set_hdr_tri_gain(ctx, feature_data, 2);
	return 0;
}

static int caymanbtele_set_hdr_tri_gain3(struct subdrv_ctx *ctx, u8 *para, u32 *len) {
	u64 *feature_data = (u64 *) para;
	caymanbtele_set_hdr_tri_gain(ctx, feature_data, 3);
	return 0;
}
...
//		}
	...
	if (set_auto_flicker(ctx, 0) || frame_length || !ctx->s_ctx.reg_addr_auto_extend)
		caymanbtele_write_frame_length(ctx, ctx->frame_length);
	/* write shutter */
	switch (exp_cnt) {
	case 1:
...
//		set_i2c_buffer(ctx, ctx->s_ctx.reg_addr_exposure_lshift, 0);
	...
	caymanbtele_lens_pos_writeback(ctx);
...
static int caymanbtele_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	caymanbtele_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
		(u64) (*(feature_data + 1)), (u64) (*(feature_data + 2)));
	return 0;
}

static void caymanbtele_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u64 values[3] = {0};
	...
	caymanbtele_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int caymanbtele_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	caymanbtele_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int caymanbtele_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	caymanbtele_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}

bool caymanbtele_set_long_exposure(struct subdrv_ctx *ctx,  u64 shutter)
{
	u16 l_shift = 0;
	u8 exposure_margin = ctx->s_ctx.mode[ctx->current_scenario_id].exposure_margin ?
		ctx->s_ctx.mode[ctx->current_scenario_id].exposure_margin : ctx->s_ctx.exposure_margin;
...
static void caymanbtele_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length)
{
	...
	if(caymanbtele_set_long_exposure(ctx, shutter)) {
		goto exit;
	}
...
static int caymanbtele_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
 	caymanbtele_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
	return 0;
}

static void caymanbtele_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter)
{
    caymanbtele_set_shutter_frame_length_convert(ctx, shutter, 0);
}

static int caymanbtele_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	caymanbtele_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], 0);
	return 0;
}
...
static int caymanbtele_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	caymanbtele_set_gain_convert(ctx, gain);
	return 0;
}

void caymanbtele_set_dummy(struct subdrv_ctx *ctx)
{
//	bool gph = !ctx->is_seamless && (ctx->s_ctx.s_gph != NULL);
//
//	if (gph)
...
static int caymanbtele_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->s_ctx.reg_addr_auto_extend ||
		...
		caymanbtele_set_dummy(ctx);
	}
	return 0;
}

void caymanbtele_set_max_framerate(struct subdrv_ctx *ctx, u16 framerate, bool min_framelength_en)
{
	u32 frame_length = 0;
...
void caymanbtele_extend_frame_length_convert(struct subdrv_ctx *ctx, u32 ns)
{
	return ;
//	int i;
//	u32 last_exp_cnt = 1;
...
static int caymanbtele_extend_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	caymanbtele_extend_frame_length_convert(ctx, ns);
	return 0;
}

bool caymanbtele_set_auto_flicker(struct subdrv_ctx *ctx, bool min_framelength_en)
{
	u16 framerate = 0;
	...
	if (framerate > 592 && framerate <= 607)
		caymanbtele_set_max_framerate(ctx, 592, min_framelength_en);
	else if (framerate > 296 && framerate <= 305)
		caymanbtele_set_max_framerate(ctx, 296, min_framelength_en);
	else if (framerate > 246 && framerate <= 253)
		caymanbtele_set_max_framerate(ctx, 246, min_framelength_en);
	else if (framerate > 236 && framerate <= 243)
		caymanbtele_set_max_framerate(ctx, 236, min_framelength_en);
	else if (framerate > 146 && framerate <= 153)
		caymanbtele_set_max_framerate(ctx, 146, min_framelength_en);
	else
		return FALSE;
	return TRUE;
}
...
static int caymanbtele_set_video_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (!framerate)
		return 0;
	caymanbtele_set_max_framerate(ctx, framerate, 0);
	caymanbtele_set_auto_flicker(ctx, 1);
	caymanbtele_set_dummy(ctx);
...
#define CAYMANBTELE_AF_READ_ID  (0x19)  //dw9800s
#define CAYMANBTELE_AF_POSITON_ADD  (0x03)
...
static bool read_af_pos(struct subdrv_ctx *ctx, u16 *positon)
{
	...
	buf[0] = CAYMANBTELE_AF_POSITON_ADD;

	msg[0].addr = CAYMANBTELE_AF_READ_ID >> 1;
	msg[0].flags = i2c_client->flags;
	msg[0].buf = buf;
	msg[0].len = 1;

	msg[1].addr  = CAYMANBTELE_AF_READ_ID >> 1;
...
static void get_imgsensor_id_from_dts(struct subdrv_ctx *ctx, u32 *sensor_id) {
	struct subdrv_entry *m_subdrv_entry = &caymanbtele_mipi_raw_entry;
