...
#define YAMAHAUWIDE_EEPROM_I2C_ADDR	(0xA2)
...
static int yamahauwide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static int yamahauwide_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter);
static int yamahauwide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yamahauwide_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length);
static int yamahauwide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, yamahauwide_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, yamahauwide_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, yamahauwide_get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, yamahauwide_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, yamahauwide_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, yamahauwide_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, yamahauwide_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, yamahauwide_streaming_resume},
	{SENSOR_FEATURE_SET_GAIN, yamahauwide_set_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, yamahauwide_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, yamahauwide_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_SEAMLESS_EXTEND_FRAME_LENGTH, yamahauwide_extend_frame_length},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, yamahauwide_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, yamahauwide_set_multi_shutter_frame_length_ctrl},
...
static struct subdrv_mode_struct mode_struct[] = {
	{ /* 3264x2448 @30FPS  */
		...
		.mode_setting_table = yamahauwide_preview_capture_setting,
		.mode_setting_len = ARRAY_SIZE(yamahauwide_preview_capture_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = yamahauwide_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(yamahauwide_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = yamahauwide_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(yamahauwide_hs_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = yamahauwide_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(yamahauwide_slim_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 288000000,
...

	{
		...
		.mode_setting_table = yamahauwide_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(yamahauwide_custom1_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = yamahauwide_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(yamahauwide_custom2_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = yamahauwide_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(yamahauwide_custom3_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = yamahauwide_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(yamahauwide_custom4_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...

	{
		...
		.mode_setting_table = yamahauwide_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(yamahauwide_custom5_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 36000000,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_step = 1,
/*	.ana_gain_table = yamahauwide_ana_gain_table,
	.ana_gain_table_size = sizeof(yamahauwide_ana_gain_table),*/
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
...
/*	.reg_addr_fast_mode = 0x3010,*/
	...
	.init_setting_table = yamahauwide_init_setting,
	.init_setting_len = ARRAY_SIZE(yamahauwide_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	...
	.control = yamahauwide_common_control,
...
struct subdrv_entry yamahauwide_mipi_raw_entry = {
	.name = SENSOR_NAME,
...
static int yamahauwide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			yamahauwide_set_shutter_convert(ctx, *(u32 *)para);
...
static unsigned int read_yamahauwide_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != yamahauwide_eeprom_info[meta_id].meta)
		return -1;

	if (size != yamahauwide_eeprom_info[meta_id].size)
		return -1;

	addr = yamahauwide_eeprom_info[meta_id].start;
	readsize = yamahauwide_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, YAMAHAUWIDE_EEPROM_I2C_ADDR >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
		...
		kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
	...
	ret = adaptor_i2c_wr_p8(ctx->i2c_client, YAMAHAUWIDE_EEPROM_I2C_ADDR >> 1,
			addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, YAMAHAUWIDE_EEPROM_I2C_ADDR >> 1, reg, 0x03);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, YAMAHAUWIDE_EEPROM_I2C_ADDR >> 1, reg, 0x02);
	}
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == YAMAHAUWIDE_SENSOR_ID)
			...
			LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
					read_cmos_eeprom_8(ctx, YAMAHAUWIDE_AESYNC_START_ADDR),
					read_cmos_eeprom_8(ctx, YAMAHAUWIDE_AESYNC_START_ADDR+1),
					read_cmos_eeprom_8(ctx, YAMAHAUWIDE_AESYNC_START_ADDR+2),
					read_cmos_eeprom_8(ctx, YAMAHAUWIDE_AESYNC_START_ADDR+3),
					read_cmos_eeprom_8(ctx, YAMAHAUWIDE_AESYNC_START_ADDR+4),
					read_cmos_eeprom_8(ctx, YAMAHAUWIDE_AESYNC_START_ADDR+5),
					read_cmos_eeprom_8(ctx, YAMAHAUWIDE_AESYNC_START_ADDR+6),
					read_cmos_eeprom_8(ctx, YAMAHAUWIDE_AESYNC_START_ADDR+7));
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
					BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, YAMAHAUWIDE_EEPROM_I2C_ADDR >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "yamahauwide read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "yamahauwide read_otp_info end\n");
}
...
void yamahauwide_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
	u32 fll_step = 0;
	u32 vblank = 0;
...
void yamahauwide_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
		enum SENSOR_SCENARIO_ID_ENUM scenario_id,
...
static int yamahauwide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return yamahauwide_set_shutter_frame_length_convert(ctx, ((u64*)para)[0], ((u64*)para)[1]);
}

static int yamahauwide_set_shutter_convert(struct subdrv_ctx *ctx, u64 shutter)
{
	return yamahauwide_set_shutter_frame_length_convert(ctx, shutter, 0);
}
...
static int yamahauwide_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	return yamahauwide_set_gain_convert(ctx, gain);
}

void yamahauwide_set_dummy(struct subdrv_ctx *ctx)
{
}
...
static int yamahauwide_extend_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	return yamahauwide_extend_frame_length_convert(ctx, ns);
}
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt)
{
