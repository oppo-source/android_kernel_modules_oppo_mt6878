...
#define DRIVER_NAME                  "gt9772b_24703"
#define GT9772B_I2C_SLAVE_ADDR        0x18

#define LOG_INF(format, args...)                                               \
	pr_info(DRIVER_NAME " [%s] " format, __func__, ##args)

#define GT9772B_NAME				 "gt9772b_24703"
...
#define GT9772B_POS_CTRL_DELAY_US     1000
/*
...
#if 0
static int gt9772b_goto_last_pos(struct gt9772b_device *gt9772b)
{
	int ret, val = g_last_pos, diff_dac, nStep_count, i;

	diff_dac = g_last_pos - GT9772B_ORIGIN_FOCUS_POS;
	if (diff_dac == 0) {
		return 0;
	}
	...
	for (i = 0; i < nStep_count; ++i) {
		...
		usleep_range(GT9772B_MOVE_DELAY_US, GT9772B_MOVE_DELAY_US + 1000);
	}
...
static int gt9772b_init(struct gt9772b_device *gt9772b)
{
	...
	if (ret)
		printk("qydebug 0xed ret:%d\n", ret);
	//printk("qydebug read 0xed value:0x%x\n", i2c_smbus_read_byte_data(client, 0xed));
mdelay(5);
	ret = i2c_smbus_write_byte_data(client, 0x06, 0x84);
	if (ret)
		printk("qydebug 0x06 ret:%d\n", ret);
	//printk("qydebug read 0x06 value:0x%x\n", i2c_smbus_read_byte_data(client, 0x06));
	
	ret = i2c_smbus_write_byte_data(client, 0x07, 0x01);
	if (ret)
		printk("qydebug 0x07 ret:%d\n", ret);
//	printk("qydebug read 0x07 value:0x%x\n", i2c_smbus_read_byte_data(client, 0x07));

	ret = i2c_smbus_write_byte_data(client, 0x08, 0x5a);
	if (ret)
		printk("qydebug 0x08 ret:%d\n", ret);
	//printk("qydebug read 0x08 value:0x%x\n", i2c_smbus_read_byte_data(client, 0x08));

	ret = i2c_smbus_read_byte_data(client, 0x00);

	LOG_INF("qydebug Check HW version: 0x%x\n", ret);

	/*ret = gt9772b_set_position(gt9772b, 800);
	if (ret) {
		printk("%s *qydebug I2C failure: %d",
			__func__, ret);
		return ret;
	}
	printk("*qydebug read 0x03 value:0x%x\n", i2c_smbus_read_word_data(client, 0x03));*/
	LOG_INF("-\n");
...
static int gt9772b_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	//struct i2c_client *client = v4l2_get_subdevdata(&gt9772b->sd);
	//gt9772b_init(gt9772b);
	//mdelay(5);
	if (ctrl->id == V4L2_CID_FOCUS_ABSOLUTE) {
		/*wait for I2C bus idle*/
		/*while (loop_time < 20)
		{
			status = i2c_smbus_read_byte_data(v4l2_get_subdevdata(&gt9772b->sd), 0x05);
			status = status & 0x01;//get reg 05 status
			printk("qydebug gt9772b 0x05 status:%x", status);
			if(status == 0){
				break;
			}
			loop_time++;
			usleep_range(GT9772B_POS_CTRL_DELAY_US, GT9772B_POS_CTRL_DELAY_US + 100);
		}*/
		LOG_INF("pos(%d)\n", ctrl->val);
...
static const struct of_device_id gt9772b_of_table[] = {
	{ .compatible = "oplus,gt9772b_24703" },
	{ },
};
