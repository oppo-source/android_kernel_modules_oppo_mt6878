...
/* get project version to distinguish wether clear ACCDET_EINT0_INVERTER_SW_EN_SFT */
...
static void dis_micbias_work_callback(struct work_struct *work)
{
	...
	if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT_IRQ)) {
		 ...
		 */
#ifndef OPLUS_ARCH_EXTENDS
/* 2024/9/2, add for hp_eint pin is not connect to gnd */
...
int fsa_ex_eint_handler(bool plug_flag)
{
        int ret = 0;
...
static int accdet_get_dts_data(void)
{
	...
	ret = of_property_read_u32(node,
			"typec_switch_type", &tmp);
	if (ret == 0) {
		if (tmp == 2) {
			#if IS_ENABLED(CONFIG_SND_SOC_FSA)
			fsa4480_register_ext_eint_handler(fsa_ex_eint_handler);
			#endif
		} else {
			#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH)
...
#ifdef OPLUS_ARCH_EXTENDS
/* get project version to distinguish wether clear ACCDET_EINT0_INVERTER_SW_EN_SFT */
static bool is_engtest_version(void)
{
	switch (get_eng_version()) {
	case ALL_NET_CMCC_TEST:
	case ALL_NET_CMCC_FIELD:
	case ALL_NET_CU_TEST:
	case ALL_NET_CU_FIELD:
	case ALL_NET_CT_TEST:
	case ALL_NET_CT_FIELD:
		pr_info("%s() The version is Eng test\n", __func__);
		return true;
	default:
		return false;
	}
...
#else /*OPLUS_ARCH_EXTENDS*/
	if (accdet_dts.headset_eint0_disable && !is_engtest_version()) {
		accdet_clear_bit(ACCDET_EINT0_INVERTER_SW_EN_ADDR, ACCDET_EINT0_INVERTER_SW_EN_SFT);
	}
#endif /*OPLUS_ARCH_EXTENDS*/
	pr_info("%s done!\n", __func__);
