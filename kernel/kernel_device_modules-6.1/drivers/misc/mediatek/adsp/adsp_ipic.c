...
// zhangyan-W8061542 add MTK ptach fix ap clear ipic irq flow  start
//bool is_adsp_recv_ipic_irq(void)
//{
//	return (readl(IPIC_CHN_INT_SEND) == 2);
//}
// zhangyan-W8061542 add MTK ptach fix ap clear ipic irq flow end
...
//		IPIC_SET(IPIC_SRC_CLR, 1 << ipic_ctrl.ap2adsp_chn);
	IPIC_SET(IPIC_SRC_CLR, 1 << ipic_ctrl.ap2adsp_chn);
	ipic_chn_status = readl(IPIC_CHN_INT_SEND);
	spin_unlock_irqrestore(&ipic_ctrl.wakelock, flags);

	if (ipic_chn_status != 0)
	pr_info("%s clear IPIC channel fail\n", __func__);
// zhangyan-W8061542 add MTK ptach fix ap clear ipic irq flow  end
}
