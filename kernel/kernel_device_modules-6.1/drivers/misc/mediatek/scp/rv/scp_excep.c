...
#include "../../adsp/include/adsp_helper.h"
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
static int scp_get_assert_log(char *p_buf, int buf_len)
{
	char *p_aee_log = NULL;
	char *p_fb_log = NULL;
	char *p_symble = NULL;
	...
	if (!p_buf || (buf_len < strlen("[ASSERT]"))) {
		pr_info("%s: input param invalid, p_buf=%p, buf_len=%d\n", __func__, p_buf, buf_len);
		return -1;
	}

	p_aee_log = scp_pickup_log_for_aee();
	if (p_aee_log) {
		p_fb_log = strstr(p_aee_log, "[ASSERT]");
		if (p_fb_log) {
			p_symble = strchr(p_fb_log, '\n');
			if (p_symble) {
				len = p_symble - p_fb_log;
				len = len < (buf_len - 1) ? len : (buf_len - 1);
				memcpy(p_buf, p_fb_log, len);
				p_buf[len] = '\0';
				pr_info("%s: len=%d, p_buf=%s\n", __func__, len, p_buf);
				return 0;
			}
		}
	}

	pr_info("%s: failed to get assert info\n", __func__);
	return -1;
}
#endif //CONFIG_OPLUS_FEATURE_MM_FEEDBACK
...
#endif
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
	char assert_info[MAX_PAYLOAD_DATASIZE] = "";
#endif
...
#endif
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
	if (scp_need_aed_dump && (get_adsp_type() == ADSP_TYPE_RV55)) {
		if (0 != scp_get_assert_log(assert_info, sizeof(assert_info))) {
			if (scpreg.core_nums == 2) {
				scnprintf(assert_info, sizeof(assert_info), "%s: core0 pc:0x%08x,lr:0x%08x;core1 pc:0x%08x,lr:0x%08x",
					scp_aed_title ? scp_aed_title : "unknown", c0_m->pc, c0_m->lr, c1_m->pc, c1_m->lr);
			} else {
				scnprintf(assert_info, sizeof(assert_info), "%s: core0 pc:0x%08x,lr:0x%08x",
					scp_aed_title ? scp_aed_title : "unknown", c0_m->pc, c0_m->lr);
			}
		}
		mm_fb_audio_kevent_named(OPLUS_AUDIO_EVENTID_ADSP_CRASH, MM_FB_KEY_RATELIMIT_5MIN,
			"FieldData@@%s$$detailData@@audio$$module@@adsp", assert_info);
	}
#endif //CONFIG_OPLUS_FEATURE_MM_FEEDBACK
...
int scp_excep_init(void)
{
	...
	/* scp reset stress */
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
/* scp_reset_stress=false, only catch scp aee dump for once;
 * scp_reset_stress=true,  catch scp aee dump for every reset.
 */
