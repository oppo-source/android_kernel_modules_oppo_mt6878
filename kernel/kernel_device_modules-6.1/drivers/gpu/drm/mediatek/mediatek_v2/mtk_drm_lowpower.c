...
extern enum mtk_drm_dataspace mtk_ovl_map_lcm_color_mode(enum mtk_drm_color_mode cm);
...
void mtk_drm_idlemgr_wb_reconfig(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *cmdq_handle)
{
	...
	enum mtk_drm_color_mode lcm_cm = 0;
	...
	if(panel_is_dunhuang()) {
		...
		mtk_plane_state->pending.prop_val[PLANE_PROP_DATASPACE] = mtk_ovl_map_lcm_color_mode(lcm_cm);
		DDPINFO("%s dataspace is %lld, lcm_cm is %d\n", __func__,
			mtk_plane_state->pending.prop_val[PLANE_PROP_DATASPACE], lcm_cm);
	}
