	...
	u32 *reason, u32 panel_width, u32 panel_height)
{
	...
	if (unlikely(mml_racing_debug))
		mml_log("[topology]Racing src(%ux%u) crop(%ux%u) dest(%ux%u) panel(%ux%u) rot %u",
			info->src.width, info->src.height,
			info->dest[0].crop.r.width, info->dest[0].crop.r.height,
			info->dest[0].data.width, info->dest[0].data.height,
			panel_width, panel_height, info->dest[0].rotate);
	...
	if (info->act_time) {
		...
		if (unlikely(mml_racing_debug == 2)) {
			ir_freq = ir_freq * 11 / 10;
			ir_freq = ir_freq * panel_width / info->dest[0].data.width;
		}
		for (i = 0; i < tp->opp_cnt; i++)
